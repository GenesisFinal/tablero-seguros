...
def fetch_ssn_inversiones_personas():
    print("[SSN Inversiones Personas] Fetching SSN Inversiones Personas Data...")
    ...
    for p in periods:
        ...
        if os.path.exists(cache_file) and time.time() - os.path.getmtime(cache_file) < 48 * 3600:
            with open(cache_file, "rb") as f:
                content = f.read()
                print(f"[SSN Inversiones Personas] Loaded cached data for period {p}")
                break
        else:
            try:
                ...
                if r.status_code == 200 and len(r.content) > 10000:
                    ...
                    with open(cache_file, "wb") as f:
                        f.write(content)
                    print(f"[SSN Inversiones Personas] Successfully downloaded period {p}")
                    break
            except Exception as e:
                print(f"[SSN Inversiones Personas] Failed period {p}: {e}")

    if not content:
        print("[SSN Inversiones Personas] Could not retrieve any SSN Inversiones Excel file.")
        return None
    ...
    try:
        ...
        if sheet_inv is None:
            print("[SSN Inversiones Personas] Sheet 'Inversiones' not found.")
            return None

        # Find row with "SEGUROS DE PERSONAS"
        start_row = None
        for i in range(len(sheet_inv)):
            val = str(sheet_inv.iloc[i, 2]).strip().upper() if pd.notnull(sheet_inv.iloc[i, 2]) else ""
            if "SEGUROS DE PERSONAS" in val or "EXCLUSIVAS PERSONAS" in val:
                start_row = i
                break
                
        if start_row is None:
            print("[SSN Inversiones Personas] Could not find row 'SEGUROS DE PERSONAS'.")
            return None
        ...
        total_personas = parse_row(sheet_inv.iloc[start_row])
        
        empresas = {
            "TODAS": {
                "nombre": "Total Seguros de Personas",
                "total_inversiones": total_personas["total_inversiones"],
                "items": total_personas["items"]
            }
        }
        ...
        while curr < len(sheet_inv):
            ...
            if not name or "TOTAL" in name or "EXCLUSIVAS RETIRO" in name or "SEGUROS DE RETIRO" in name:
                break
            
            parsed_c = parse_row(sheet_inv.iloc[curr])
            display_name = name.title()
            if "SEGUNDA" in name:
                display_name = "La Segunda Personas"
        ...
        print(f"[SSN Inversiones Personas] Parsed successfully ({len(empresas)-1} companies).")
        return result

    except Exception as e:
        print(f"[SSN Inversiones Personas] Error parsing SSN Inversiones Excel: {e}")
        import traceback
        traceback.print_exc()
        return None

if __name__ == "__main__":
    data = fetch_ssn_inversiones_personas()
